...
class Store extends BasicStore<Data> {
  ...
  async init() {
    ...
    this.setData({
      ...
      listTitle: get(fresnsConfig, "post_name"),
    });
    await this.loadData();
  }
  ...
  addListener() {
    eventManager.on("onChangeComment", this.onChangePost);
    eventManager.on("onChangeUser", this.onChangeUser);
  }

  removeListener() {
    eventManager.off("onChangeComment", this.onChangePost);
